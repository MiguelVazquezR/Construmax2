    ...
    <div class="min-h-screen bg-gray-100 print:bg-white p-4 print:p-0 font-sans text-xs text-gray-900 dark:text-gray-900">
        ...
        <div class="max-w-5xl mx-auto bg-white dark:bg-white print:shadow-none shadow-lg p-8 print:p-0 text-gray-900 dark:text-gray-900">
            ...
            <div class="flex border-2 border-blue-900 mb-2">
                <!-- Logo Area -->
                <div class="w-1/3 flex justify-center items-center border-r-2 border-blue-900 p-1 bg-white dark:bg-white">
                ...
                <div class="w-2/3 flex flex-col">
                    ...
                    <div class="flex border-b border-blue-900 flex-1">
                        ...
                        <div class="w-3/4 px-2 py-1.5 text-center font-bold text-red-600 dark:text-red-600 flex items-center justify-center truncate">{{ budget.ticket?.branch?.branch_name || 'N/D' }}</div>
                    </div>
                    
                    <div class="flex flex-1">
                        <div class="w-1/4 bg-[#5b9bd5] text-white px-2 py-1.5 font-bold border-r border-blue-900 flex items-center">Región</div>
                        <div class="w-1/4 px-2 py-1.5 text-center font-bold text-red-600 dark:text-red-600 border-r border-blue-900 flex items-center justify-center uppercase">{{ budget.ticket?.branch?.region || 'N/D' }}</div>
                        <div class="w-1/4 bg-[#5b9bd5] text-white px-2 py-1.5 font-bold border-r border-blue-900 flex items-center">País</div>
                        <div class="w-1/4 px-2 py-1.5 text-center font-bold text-red-600 dark:text-red-600 flex items-center justify-center uppercase">{{ budget.ticket?.branch?.country || 'N/D' }}</div>
                    </div>
                </div>
            </div>

            <!-- Provider & General Data -->
            <div class="flex border-2 border-blue-900 border-b text-[11px] mt-2 bg-blue-50 dark:bg-blue-50">
                <div class="w-[15%] bg-[#5b9bd5] text-white px-2 py-1 font-bold border-r border-blue-900">Proveedor / Cliente</div>
                <div class="w-[35%] px-2 py-1 text-center font-bold border-r border-blue-900 text-gray-900 dark:text-gray-900">{{ budget.ticket?.customer?.name || 'N/D' }}</div>
                <div class="w-[10%] bg-[#5b9bd5] text-white px-2 py-1 font-bold border-r border-blue-900 text-center">RFC</div>
                <div class="w-[20%] px-2 py-1 text-center font-bold border-r border-blue-900 text-gray-900 dark:text-gray-900">{{ budget.ticket?.customer?.rfc || 'N/D' }}</div>
                <div class="w-[10%] bg-[#5b9bd5] text-white px-2 py-1 font-bold border-r border-blue-900 text-center">No. Reporte</div>
                <div class="w-[10%] px-2 py-1 text-center font-bold text-gray-900 dark:text-gray-900">{{ budget.ticket?.folio || budget.ticket?.id }}</div>
            </div>

            <div class="flex border-2 border-t-0 border-blue-900 text-[11px] bg-blue-50 dark:bg-blue-50 mb-4">
                <div class="w-[15%] bg-[#5b9bd5] text-white px-2 py-1 font-bold border-r border-blue-900">Servicio</div>
                <div class="w-[35%] px-2 py-1 text-center font-bold border-r border-blue-900 uppercase text-gray-900 dark:text-gray-900">{{ budget.ticket?.service_type || 'N/D' }}</div>
                ...
                <div class="w-[20%] px-2 py-1 text-center font-bold border-r border-blue-900 flex items-center justify-center text-gray-900 dark:text-gray-900">{{ formatDate(budget.ticket?.scheduled_start) }}</div>
                ...
                <div class="w-[10%] px-2 py-1 text-center font-bold flex items-center justify-center text-gray-900 dark:text-gray-900">{{ formatDate(budget.ticket?.scheduled_end) }}</div>
            </div>

            <!-- Items Table -->
            <table class="w-full text-[11px] border-collapse border-2 border-blue-900 text-gray-900 dark:text-gray-900">
                <thead>
                ...
                <tbody>
                    <tr v-for="(item, index) in catalog?.items" :key="item.id" class="text-center group">
                        <td class="border border-blue-900 py-1.5 px-1 text-gray-900 dark:text-gray-900">{{ index + 1 }}</td>
                        <td class="border border-blue-900 py-1.5 px-2 text-left leading-tight break-words text-gray-900 dark:text-gray-900">{{ item.description }}</td>
                        <td class="border border-blue-900 py-1.5 px-1 text-gray-900 dark:text-gray-900">{{ item.unit }}</td>
                        <td class="border border-blue-900 py-1.5 px-1 text-gray-900 dark:text-gray-900">{{ Number(item.quantity).toFixed(2) }}</td>
                        <td class="border border-blue-900 py-1.5 px-2 text-right text-gray-900 dark:text-gray-900">
                            <div class="flex justify-between w-full"><span>$</span><span>{{ formatCurrency(item.unit_price) }}</span></div>
                        </td>
                        <td class="border border-blue-900 py-1.5 px-2 text-right bg-blue-50 dark:bg-blue-50 text-gray-900 dark:text-gray-900">
                    ...
                    <tr v-if="!catalog?.items?.length">
                        <td colspan="6" class="border border-blue-900 py-4 text-center text-gray-400 dark:text-gray-400">Sin partidas registradas en esta versión.</td>
                    </tr>
                </tbody>
                <tfoot v-if="catalog">
                    <tr>
            ...
            <div class="mt-2 text-[10px] text-right font-bold text-blue-900 dark:text-blue-900 uppercase">
