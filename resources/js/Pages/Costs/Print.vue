...
<template>
    ...
    <div class="min-h-screen bg-gray-100 print:bg-white p-4 print:p-0 font-sans text-xs text-gray-900 dark:text-gray-900">
        ...
        <div class="max-w-5xl mx-auto bg-white dark:bg-white print:shadow-none shadow-lg p-8 print:p-0 text-gray-900 dark:text-gray-900">
            ...
            <div class="flex border-2 border-[#1e1e20] mb-2">
                ...
                <div class="w-2/3 flex flex-col">
                    ...
                    <div class="flex border-b border-[#1e1e20] flex-1">
                        <div class="w-1/4 bg-[#1e1e20] text-white px-2 py-1.5 font-bold border-r border-[#1e1e20] flex items-center">Nombre</div>
                    ...
                    <div class="flex border-b border-[#1e1e20] flex-1">
                        <div class="w-1/4 bg-[#1e1e20] text-white px-2 py-1.5 font-bold border-r border-[#1e1e20] flex items-center">Ciudad</div>
                    ...
                    <div class="flex flex-1">
                        <div class="w-1/4 bg-[#1e1e20] text-white px-2 py-1.5 font-bold border-r border-[#1e1e20] flex items-center">Estado</div>
                        <div class="w-1/4 px-2 py-1.5 text-center font-bold text-[#f26c17] border-r border-[#1e1e20] flex items-center justify-center uppercase">{{ budget.ticket?.branch?.region || 'N/D' }}</div>
                        <div class="w-1/4 bg-[#1e1e20] text-white px-2 py-1.5 font-bold border-r border-[#1e1e20] flex items-center">País</div>
            ...
            <div class="flex border-2 border-[#1e1e20] border-b text-[11px] mt-2 bg-orange-50">
                <div class="w-[15%] bg-[#1e1e20] text-white px-2 py-1 font-bold border-r border-[#1e1e20]">Proveedor / Cliente</div>
                <div class="w-[35%] px-2 py-1 text-center font-bold border-r border-[#1e1e20] text-gray-900">{{ budget.ticket?.customer?.name || 'N/D' }}</div>
                <div class="w-[10%] bg-[#1e1e20] text-white px-2 py-1 font-bold border-r border-[#1e1e20] text-center">RFC</div>
                <div class="w-[20%] px-2 py-1 text-center font-bold border-r border-[#1e1e20] text-gray-900">{{ budget.ticket?.customer?.rfc || 'N/D' }}</div>
                <div class="w-[10%] bg-[#1e1e20] text-white px-2 py-1 font-bold border-r border-[#1e1e20] text-center">No. Reporte</div>
                <div class="w-[10%] px-2 py-1 text-center font-bold text-gray-900">{{ budget.ticket?.folio || budget.ticket?.id }}</div>
            </div>

            <div class="flex border-2 border-t-0 border-[#1e1e20] text-[11px] bg-orange-50 mb-4">
                <div class="w-[15%] bg-[#1e1e20] text-white px-2 py-1 font-bold border-r border-[#1e1e20]">Servicio</div>
                <div class="w-[35%] px-2 py-1 text-center font-bold border-r border-[#1e1e20] uppercase text-gray-900">{{ budget.ticket?.service_type || 'N/D' }}</div>
                <div class="w-[10%] bg-[#1e1e20] text-white px-2 py-1 font-bold border-r border-[#1e1e20] text-center flex items-center justify-center">Fecha inicio</div>
                <div class="w-[20%] px-2 py-1 text-center font-bold border-r border-[#1e1e20] flex items-center justify-center text-gray-900">{{ formatDate(budget.ticket?.scheduled_start) }}</div>
                <div class="w-[10%] bg-[#1e1e20] text-white px-2 py-1 font-bold border-r border-[#1e1e20] text-center flex items-center justify-center">Fecha fin</div>
            ...
            <!-- Items Table -->
            <table class="w-full text-[11px] border-collapse border-2 border-[#1e1e20] text-gray-900">
                <thead>
                    <tr class="bg-[#f26c17] text-white text-center">
                        <th class="border border-[#1e1e20] py-2 w-16">PARTIDA<br>CLAVE</th>
                        <th class="border border-[#1e1e20] py-2">DESCRIPCIÓN</th>
                        <th class="border border-[#1e1e20] py-2 w-20">UNIDAD</th>
                        <th class="border border-[#1e1e20] py-2 w-20">CANTIDAD</th>
                        <th class="border border-[#1e1e20] py-2 w-24">P.U.</th>
                ...
                <tbody>
                    <tr v-for="(item, index) in catalog?.items" :key="item.id" class="text-center">
                        ...
                        <td class="border border-[#1e1e20] py-1.5 px-2 text-left leading-tight break-words text-gray-900">{{ item.description }}</td>
                        <td class="border border-[#1e1e20] py-1.5 px-1 text-gray-900">{{ item.unit }}</td>
                        <td class="border border-[#1e1e20] py-1.5 px-1 text-gray-900">{{ Number(item.quantity).toFixed(2) }}</td>
                    ...
                    <tr v-if="!catalog?.items?.length">
                        <td colspan="6" class="border border-[#1e1e20] py-4 text-center text-gray-400">Sin partidas registradas en esta versión.</td>
                    </tr>
                </tbody>
                <tfoot v-if="catalog">
                    <tr>
                        <td colspan="4" class="border-t-2 border-[#1e1e20] border-b-0 border-l-0"></td>
                        <td class="border border-[#1e1e20] py-1.5 px-2 font-bold text-white bg-[#1e1e20] text-center">SUB-TOTAL</td>
                    ...
                    <tr>
                        <td colspan="4" class="border-none"></td>
                        <td class="border border-[#1e1e20] py-1.5 px-2 font-bold text-white bg-[#1e1e20] text-center">TOTAL</td>
