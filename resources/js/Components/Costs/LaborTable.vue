...
<template>
    <div class="bg-white dark:bg-[#1e1e20] rounded-lg shadow-sm border border-gray-100 dark:border-[#2b2b2e]">
        <div class="p-4 border-b border-gray-100 dark:border-[#2b2b2e] flex justify-between items-center">
            ...
            <el-button v-if="canEdit" type="warning" plain size="small" icon="Plus" @click="$emit('add')">
                ...
                <tbody>
                    <tr v-for="(item, index) in items" :key="'l'+index" class="border-b dark:border-[#2b2b2e]">
                        ...
                        <td class="px-2 py-2">
                            <el-input v-model="item.description" type="textarea" :rows="2"
                                placeholder="Descripción de la labor" :disabled="!canEdit" />
                        </td>
                        <td class="px-2 py-2">
                            <el-input v-model="item.technician" placeholder="Núm. de técnicos" :disabled="!canEdit" />
                        </td>
                        <td class="px-2 py-2">
                            <el-input-number v-model="item.hours" :min="0" :step="0.5" :controls="false"
                                class="!w-full text-right" @update:model-value="onRowTotal(item)" :disabled="!canEdit" />
                        </td>
                        <td class="px-2 py-2">
                    ...
                    <tr v-if="items.length === 0">
                        <td colspan="7" class="px-4 py-6 text-center text-gray-400">
                            {{ canEdit ? 'No hay mano de obra. Haz clic en "Agregar mano de obra".' : 'Sin mano de obra registrada.' }}
                        </td>
                    </tr>
                </tbody>
