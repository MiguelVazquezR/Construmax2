...
| Migrations | `database/migrations/2026_09_19_000001` … `000015` + `2026_09_20_000001` | 16 migrations: settings → profiles → devices → logs → overrides → shifts → assignments → holidays → vacations → incidents → periods → adjustments → payslips (3 tables) → `expenses.payroll_period_id` → face enrollments → payroll notes |
...
## Test coverage (`tests/Feature/Payroll/`, 23 files, 210 tests)
