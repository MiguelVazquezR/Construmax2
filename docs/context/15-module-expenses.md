...
- **Filters (server-side):** `search` (concept or reference), `folio`, `status`, `category_id`, `payment_method`, `type` (`general` / `budget` / `deposit` — the *Comisiones* option was removed from the index filter sep 18 2026; the backend still accepts `commission`), `budget_id`, `from` / `to` (date range). 15 rows per page. The `category_id` filter also matches concept payments without their own category whose budget concept carries the matching cost type — so filtering by the default *Mano de obra* / *Materiales* categories returns exactly what the Categoría column displays.
